...
// ORIGINAL float B_gyro = 0.1;       //Gyro LP filter paramter, (MPU6050 default: 0.1. MPU9250 default: 0.17)
// J.M. 20220210
...
//J.M. PID parameters are in an array now
// Index[0] hover, Index[1] transition, Index[2] Fly Forward, Index[3] actual



//                         hover   trans   ff      actual
float Kp_roll_angle[]  = { 0.03,   0.03,   0.03,   0.08};   //Roll P-gain - angle mode                                         
float Ki_roll_angle[]  = { 0.025,  0.015,  0.04,   0.04};   //Roll I-gain - angle mode                                        
float Kd_roll_angle[]  = { 0.005,  0.005,  0.005,  0.005};   //Roll D-gain - angle mode (if using controlANGLE2(), has no effect. Use B_loop_roll)                                        
float B_loop_roll[]    = { 0.9,    0.9,    0.9,    0.9};    //Roll damping term for controlANGLE2(), lower is more damping (must be between 0 to 1)                                        
float Kp_pitch_angle[] = { 0.03,   0.03,   0.03,   0.09};   //Pitch P-gain - angle mode                                        
float Ki_pitch_angle[] = { 0.03,   0.04,   0.04,   0.07};   //Pitch I-gain - angle mode                                        
float Kd_pitch_angle[] = { 0.001,  0.005,  0.005,  0.005};   //Pitch D-gain - angle mode (if using controlANGLE2(), has no effect. Use B_loop_pitch)                                        
float B_loop_pitch[]   = { 0.9,    0.9,    0.9,    0.9};    //Pitch damping term for controlANGLE2(), lower is more damping (must be between 0 to 1)                                        
                                                                                                   
float Kp_roll_rate[]   = { 0.20,   0.3,    0.5,    0.08};   //Roll P-gain - rate mode                                        
float Ki_roll_rate[]   = { 0.25,   0.2,    0.4,    0.05};   //Roll I-gain - rate mode                                        
float Kd_roll_rate[]   = { 0.0001, 0.0001, 0.0001, 0.0001}; //Roll D-gain - rate mode (be careful when increasing too high, motors will begin to overheat!)                                        
float Kp_pitch_rate[]  = { 0.25,    0.6,    0.5,    0.08};   //Pitch P-gain - rate mode                                        
float Ki_pitch_rate[]  = { 0.35,    0.5,    0.5,    0.08};   //Pitch I-gain - rate mode                                        
float Kd_pitch_rate[]  = { 0.0001, 0.0001, 0.0001, 0.0001}; //Pitch D-gain - rate mode (be careful when increasing too high, motors will begin to overheat!)                                        
                                                                                                   
float Kp_yaw[]         = { 1.1,    1.0,    0.5,    0.05};   //Yaw P-gain                                        
float Ki_yaw[]         = { 0.9,    0.9,    0.5,    0.05};   //Yaw I-gain                                        
float Kd_yaw[]         = { 0.0001, 0.0001, 0.0001, 0.0001}; //Yaw D-gain (be careful when increasing too high, motors will begin to overheat!)                                        
                                                    
float maxRoll[]        = {  30.0,   60.0,   60.0,   60.0};  //Max roll angle in degrees for angle mode (maximum 60 degrees), deg/sec for rate mode 
float maxPitch[]       = {  30.0,   60.0,   60.0,   60.0};  //Max pitch angle in degrees for angle mode (maximum 60 degrees), deg/sec for rate mode
float maxYaw[]         = {  90.0,   60.0,   60.0,   60.0};  //Max yaw rate in deg/sec
...
unsigned long print_counter, serial_counter, servo_counter;
...
//J.M. 20220317 servo jitter prevention
unsigned long S1_command_prev = 1500,S2_command_prev = 1500,S3_command_prev = 1500,S4_command_prev = 1500,S5_command_prev = 1500;
unsigned long S1_command_integr =0,S2_command_integr =0,S3_command_integr =0,S4_command_integr =0,S5_command_integr =0;
unsigned long S1_command_tresh =5,S2_command_tresh =5,S3_command_tresh =5,S4_command_tresh =5,S5_command_tresh =5;
...
//J.M. definitions for transient-limits for motor-commands
float m2Fader, m3Fader, m1_command_scaled_prev, m2_command_scaled_prev, m3_command_scaled_prev; // m4_command_scaled_prev, m5_command_scaled_prev, m6_command_scaled_prev;
float m2Fader_des = 1;
float m3Fader_des = 1;
int mode_change_detect = 0;
int in_mode = 1;
int in_mode_prev =0;
// end J.M.
...
float nose_fader_prev = 0.0;

  //Servo trims - tune accordingly // J.M. moved to be global variables - otherwise the local variables will be set every loop.
  float tilt_servo_trim = 0.55; //adjust to center tilt servo
  
  float left_elevon_trim = 0.45; //adjust in 0.1 incriments to bring left elevon level  0.48 decrease means fly up
  float right_elevon_trim = 0.53; //adjust in 0.1 incriments to bring right elevon level  0.5 decrease means fly down
  
  float left_ail_trim_hover = 0.19; //adjust in 0.1 incriments to bring left aileron to ~70 degrees down when in hover mode
  float right_ail_trim_hover = 0.87; //adjust in 0.1 incriments to bring right aileron to ~70 degrees down when in hover mode
  
  float left_ail_trim_trans = 0.49; //adjust in 0.1 incriments to bring left aileron to ~30 degrees down when in transition mode
  float right_ail_trim_trans = 0.57; //adjust in 0.1 incriments to bring right aileron to ~30 degrees down when in transition mode
  
  float left_ail_trim_ff = 0.77; //adjust in 0.1 incriments to bring left aileron to level when in forward flight mode -> increase means more vertical propeller
  float right_ail_trim_ff = 0.34; //adjust in 0.1 incriments to bring right aileron to level when in forward flight mode -> decrease means more vertical propeller

  float elevon_gain = 0.8; //adjust to increase/decrease throw of elevon servos in forward flight
  float aileron_gain = 0.2; //adjust to increase/decrease throw of aileron servos in forward flight
...
void setup() {
  ...
  delay(10);
  // J.M.
  m1_command_scaled_prev = m2_command_scaled_prev = m3_command_scaled_prev = /*m4_command_scaled_prev = m5_command_scaled_prev = m6_command_scaled_prev = */ 0.0;
...
void loop() {
 ...
 // printLoopRate();      //prints the time between loops in microseconds (expected: microseconds between loop iterations)
  ...
  // J.M. CALL FLIGHTMODES INSTEAD OF CONTROLMIXER
	if (in_mode == 1) {
		flightModeHover();
	}
	else if (in_mode == 2) {
	...
	else {flightModeHover();		//should never happen
	}
	//J.M. End
  
  
  //J.M. Limit transients to motor (because they were overheating during the very first seconds of the first powering-up)
  limitMotorTransients();
  ...
  //J.M. 20220210 we will not output every cycle to servos...
  commandServos(); //Sends command pulses to servos every time defined by servo_counter
...
void getIMUdata() {
 ...
 //Accelerometer
  ...
  GyroY_prev = GyroY;
  GyroZ_prev = GyroZ;
  //J.M. added deadband for gyro-data
  //if ((GyroX < 0.1) && (GyroX > -0.1)) GyroX = 0;
  //if ((GyroY < 0.1) && (GyroY > -0.1)) GyroY = 0;
  //if ((GyroZ < 0.1) && (GyroZ > -0.1)) GyroZ = 0;
  // END J.M.
  //Magnetometer
  MagX = MgX/6.0; //uT
  MagY = MgY/6.0;
...
void getDesState() {
  ...
  roll_des = constrain(roll_des, -1.0, 1.0)*maxRoll[3]; //between -maxRoll and +maxRoll
  pitch_des = constrain(pitch_des, -1.0, 1.0)*maxPitch[3]; //between -maxPitch and +maxPitch
  yaw_des = constrain(yaw_des, -1.0, 1.0)*maxYaw[3]; //between -maxYaw and +maxYaw


  roll_passthru = roll_des/(2*maxRoll[3]);
  pitch_passthru = pitch_des/(2*maxPitch[3]);
  yaw_passthru = yaw_des/(2*maxYaw[3]);
}
...
void controlANGLE() {
  ...
  roll_PID = 0.01*(Kp_roll_angle[3]*error_roll + Ki_roll_angle[3]*integral_roll - Kd_roll_angle[3]*derivative_roll); //scaled by .01 to bring within -1 to 1 range
  ...
  pitch_PID = .01*(Kp_pitch_angle[3]*error_pitch + Ki_pitch_angle[3]*integral_pitch - Kd_pitch_angle[3]*derivative_pitch); //scaled by .01 to bring within -1 to 1 range
  ...
  yaw_PID = .01*(Kp_yaw[3]*error_yaw + Ki_yaw[3]*integral_yaw + Kd_yaw[3]*derivative_yaw); //scaled by .01 to bring within -1 to 1 range
...
void controlANGLE2() {
  ...
  roll_des_ol = Kp_roll_angle[3]*error_roll + Ki_roll_angle[3]*integral_roll_ol - Kd_roll_angle[3]*derivative_roll;
  ...
  pitch_des_ol = Kp_pitch_angle[3]*error_pitch + Ki_pitch_angle[3]*integral_pitch_ol - Kd_pitch_angle[3]*derivative_pitch;
  ...
  roll_des_ol = (1.0 - B_loop_roll[3])*roll_des_prev + B_loop_roll[3]*roll_des_ol;
  pitch_des_ol = (1.0 - B_loop_pitch[3])*pitch_des_prev + B_loop_pitch[3]*pitch_des_ol;
  ...
  roll_PID = .01*(Kp_roll_rate[3]*error_roll + Ki_roll_rate[3]*integral_roll_il + Kd_roll_rate[3]*derivative_roll); //scaled by .01 to bring within -1 to 1 range
  ...
  pitch_PID = .01*(Kp_pitch_rate[3]*error_pitch + Ki_pitch_rate[3]*integral_pitch_il + Kd_pitch_rate[3]*derivative_pitch); //scaled by .01 to bring within -1 to 1 range
...
void controlRATE() {
  ...
  integral_roll = constrain(integral_roll, -i_limit, i_limit); //saturate integrator to prevent unsafe buildup
  derivative_roll = (error_roll - error_roll_prev)/dt; 
  roll_PID = .01*(Kp_roll_rate[3]*error_roll + Ki_roll_rate[3]*integral_roll + Kd_roll_rate[3]*derivative_roll); //scaled by .01 to bring within -1 to 1 range
  ...
  integral_pitch = constrain(integral_pitch, -i_limit, i_limit); //saturate integrator to prevent unsafe buildup
  derivative_pitch = (error_pitch - error_pitch_prev)/dt; 
  pitch_PID = .01*(Kp_pitch_rate[3]*error_pitch + Ki_pitch_rate[3]*integral_pitch + Kd_pitch_rate[3]*derivative_pitch); //scaled by .01 to bring within -1 to 1 range
...
void flightModeHover() {
  //Flight mode 1 - Hover
    ...
    nose_fader = floatFaderLinear(nose_fader, nose_fader_prev, 1.0, 1.0 /*0.4*/, 1, 2000);
  ...
  // m_fader = 0;
    ...
    s3_holder = floatFaderLinear2(s3_holder, right_ail_trim_hover, right_ail_trim_trans, right_ail_trim_hover, 2.0, 2.0, 2000); //holder to handle the constant servo trim offset for right aileron
    s4_holder = floatFaderLinear2(s4_holder, left_ail_trim_hover, left_ail_trim_hover, left_ail_trim_trans, 2.0, 2.0, 2000); //holder to handle the constant servo trim offset for left aileron
    s3_command_scaled = s3_holder - 0.0003*pitch_PID - 0.2*yaw_PID; //right aileron
    s4_command_scaled = s4_holder + 0.0003*pitch_PID - 0.2*yaw_PID; //left aileron
    s5_command_scaled = tilt_servo_trim + yaw_PID; //yaw tilt
	}

//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void flightModeTrans() {
  //Flight mode 2 - Transition

 	
    nose_fader = floatFaderLinear(nose_fader, nose_fader_prev, 1.0, 1.0 /*0.4*/, 1, 2000); //1.0 = standard hover control, 0.0 = nose propeller off. This function call fades from 0.0 to 1.0 in 0.4 seconds
   /* //J.M.
    m2Fader = floatFaderLinear(m2Fader, m2Fader_des, 1.0, 2.0, 1, 2000);
    m3Fader = floatFaderLinear(m3Fader, m3Fader_des, 1.0, 2.0, 1, 2000);

    //end J.M. */ 
    m1_command_scaled = nose_fader*(thro_des - pitch_PID); //nose propeller
    m2_command_scaled = thro_des + m2Fader * (pitch_PID - roll_PID - 1 * yaw_PID); //right propeller
    m3_command_scaled = thro_des + m3Fader * (pitch_PID + roll_PID + 1 * yaw_PID); //left propeller
    ...
    s3_holder = floatFaderLinear2(s3_holder, right_ail_trim_trans, right_ail_trim_ff, right_ail_trim_hover, 2.0, 2.0, 2000); //holder to handle the constant servo trim offset for right aileron
    s4_holder = floatFaderLinear2(s4_holder, left_ail_trim_trans, left_ail_trim_hover, left_ail_trim_ff, 2.0, 2.0, 2000); //holder to handle the constant servo trim offset for left aileron
...
void flightModeFF(){
  //Flight mode 3 - Forward Flight 
    ...
    nose_fader = floatFaderLinear(nose_fader, 0.0,nose_fader_prev, 1.0 /*0.6*/, 0, 2000); //1.0 = standard hover control, 0.0 = nose propeller off. This function call fades from 1.0 to 0.0 in 0.6 seconds
    //J.M.
    //m_fader = 0;
    m1_command_scaled = nose_fader*(thro_des - pitch_PID); //nose propeller
    ...
    s1_command_scaled = right_elevon_trim + elevon_gain*(roll_PID - pitch_PID); //right elevon
    //s1_command_scaled = roll_passthru + 0.5; //direct output from transmitter for testing possible servoangle 700 - 2300 ms works
    s2_command_scaled = left_elevon_trim + elevon_gain*(roll_PID + pitch_PID); //left elevon
    s3_holder = floatFaderLinear2(s3_holder, right_ail_trim_ff, right_ail_trim_ff, right_ail_trim_hover, 2.0, 2.0, 2000);
    s4_holder = floatFaderLinear2(s4_holder, left_ail_trim_ff, left_ail_trim_hover, left_ail_trim_ff, 2.0, 2.0, 2000);
    s3_command_scaled = s3_holder - aileron_gain*(roll_PID - pitch_PID); //right aileron
    s4_command_scaled = s4_holder - aileron_gain*(roll_PID + pitch_PID); //left aileron
...
// J.M. to prevent too sharp transients to motors to prevent overheating.
void limitMotorTransients() {
  float transient_limit = 0.01; //should be set to 0.01 while tuning the PIDs and then reduced to a value as small as possible and as high that there is no effect on the tuned PIDs
  if (m1_command_scaled > (m1_command_scaled_prev +transient_limit)) {
     m1_command_scaled_prev=m1_command_scaled = m1_command_scaled_prev +transient_limit;
   }
  else if (m1_command_scaled < (m1_command_scaled_prev -transient_limit)) {
    m1_command_scaled_prev=m1_command_scaled = m1_command_scaled_prev -transient_limit;
  }
  else {
    m1_command_scaled_prev=m1_command_scaled;  
  }
  if (m2_command_scaled > (m2_command_scaled_prev +transient_limit)) {
    m2_command_scaled_prev=m2_command_scaled = m2_command_scaled_prev +transient_limit;
  }
  else if (m2_command_scaled < (m2_command_scaled_prev -transient_limit)) {
    m2_command_scaled_prev=m2_command_scaled = m2_command_scaled_prev -transient_limit;
  }
  else {
    m2_command_scaled_prev=m2_command_scaled;  
  }
  if (m3_command_scaled > (m3_command_scaled_prev +transient_limit)) {
    m3_command_scaled_prev=m3_command_scaled = m3_command_scaled_prev +transient_limit;
  }
  else if (m3_command_scaled < (m3_command_scaled_prev -transient_limit)) {
    m3_command_scaled_prev=m3_command_scaled = m3_command_scaled_prev -transient_limit;
  }
  else {
...
void commandMotors() {
  ...
  while (wentLow < 3 ) { //keep going until final (6th) pulse is finished, then done
    timer = micros();
...
float floatFaderLinear2(float param, float param_des, float param_lower, float param_upper, float fadeTime_up, float fadeTime_down, int loopFreq){
  ...
  float divFaktor_down = 1;
  if (fadeTime_down == 0.0){
	  divFaktor_down = 1;
  }
  else {divFaktor_down = (fadeTime_down*loopFreq);
  }
  float divFaktor_up = 1;
  if (fadeTime_up == 0.0){
	  divFaktor_up = 1;
  }
  else {divFaktor_up = (fadeTime_up*loopFreq);
  }
  if (param > param_des) { //need to fade down to get to desired
    float diffParam = (param_upper - param_des)/divFaktor_down;
    param = param - diffParam;
  }
  else if (param < param_des) { //need to fade up to get to desired
    float diffParam = (param_des - param_lower)/divFaktor_up;
    param = param + diffParam;
  }
...
// J.M. 20220210 added subroutine
void commandServos() {
  if (current_time - servo_counter > 3000) {

  S1_command_integr = S1_command_integr + s1_command_PWM - S1_command_prev;
  S2_command_integr = S2_command_integr + s2_command_PWM - S2_command_prev;
  S3_command_integr = S3_command_integr + s3_command_PWM - S3_command_prev;
  S4_command_integr = S4_command_integr + s4_command_PWM - S4_command_prev;
  S5_command_integr = S5_command_integr + s5_command_PWM - S5_command_prev;
  if (abs(S1_command_integr)>S1_command_tresh){
    S1_command_prev = s1_command_PWM = S1_command_prev + S1_command_integr;
    S1_command_integr = 0;
  }
  else {
    s1_command_PWM = S1_command_prev;
  }

  
  if (abs(S2_command_integr)>S2_command_tresh){
    S2_command_prev = s2_command_PWM = S2_command_prev + S2_command_integr;
    S2_command_integr = 0;
  }
  else {
    s2_command_PWM = S2_command_prev;
  }


  if (abs(S3_command_integr)>S3_command_tresh){
    S3_command_prev = s3_command_PWM = S3_command_prev + S3_command_integr;
    S3_command_integr = 0;
  }
  else {
    s3_command_PWM = S3_command_prev;
  }


if (abs(S4_command_integr)>S4_command_tresh){
    S4_command_prev = s4_command_PWM = S4_command_prev + S4_command_integr;
    S4_command_integr = 0;
  }
  else {
    s4_command_PWM = S4_command_prev;
  }


if (abs(S5_command_integr)>S5_command_tresh){
    S5_command_prev = s5_command_PWM = S5_command_prev + S5_command_integr;
    S5_command_integr = 0;
  }
  else {
    s5_command_PWM = S5_command_prev;
  }
 ...
 servo_counter = micros();
  }
}

   void selectFlightMode() {
    ...
    if ((in_mode - in_mode_prev)!= 0) {
      mode_change_detect = 1;
	  nose_fader_prev = nose_fader;
	  copyPIDparameters();	// copy the pid-parameters from mode 1, 2 or 3 to "active" (index [3])
	  resetPIDintegrals();  // to provide bumpless transfer from one control mode to the other
	  resetPIDerrors();		// to provide bumpless transfer from one control mode to the other

	}
	  else {mode_change_detect = 0;
	  }
	  in_mode_prev = in_mode;
   // J.M. End
      
	   
   }

void copyPIDparameters() {
	
	  
	// J.M. from flightmode ( 1, 2 or 3) to actual
	  Kp_roll_angle[3]  = Kp_roll_angle[(in_mode-1)]  ;
	  Ki_roll_angle[3]  = Ki_roll_angle[(in_mode-1)]  ;
	  Kd_roll_angle[3]  = Kd_roll_angle[(in_mode-1)]  ;
	  B_loop_roll[3]	  = B_loop_roll[(in_mode-1)]    ;
	  Kp_pitch_angle[3] = Kp_pitch_angle[(in_mode-1)] ;
	  Ki_pitch_angle[3] = Ki_pitch_angle[(in_mode-1)] ;
	  Kd_pitch_angle[3] = Kd_pitch_angle[(in_mode-1)] ;
	  B_loop_pitch[3]	  = B_loop_pitch[(in_mode-1)]   ;
	                                       
	  Kp_roll_rate[3]   = Kp_roll_rate[(in_mode-1)]   ;
	  Ki_roll_rate[3]   = Ki_roll_rate[(in_mode-1)]   ;
	  Kd_roll_rate[3]   = Kd_roll_rate[(in_mode-1)]   ;
	  Kp_pitch_rate[3]  = Kp_pitch_rate[(in_mode-1)]  ;
	  Ki_pitch_rate[3]  = Ki_pitch_rate[(in_mode-1)]  ;
	  Kd_pitch_rate[3]  = Kd_pitch_rate[(in_mode-1)]  ;
	                                       
	  Kp_yaw[3]         = Kp_yaw[(in_mode-1)]         ;     
	  Ki_yaw[3]         = Ki_yaw[(in_mode-1)]         ;     
	  Kd_yaw[3]         = Kd_yaw[(in_mode-1)]         ; 
	  
	  maxRoll[3]        = maxRoll[(in_mode-1)]        ;
	  maxPitch[3]       = maxPitch[(in_mode-1)]       ;
	  maxYaw[3]         = maxYaw[(in_mode-1)]         ;
	  
}	  

void resetPIDintegrals() {
integral_pitch = integral_pitch_prev = integral_roll = integral_roll_prev = integral_yaw = integral_yaw_prev = 0.0;

}

void resetPIDerrors() {
error_pitch = error_pitch_prev = error_roll = error_roll_prev = error_yaw = error_yaw_prev = 0;
...
void printDesiredState() {
  if (current_time - print_counter > 10000) {
    ...
    //Serial.print(F("ReTu roll: "));
    //Serial.print(roll_D_remote);
    //Serial.print(F("ReTu pit: "));
    //Serial.print(pitch_D_remote);
    //Serial.print(F("ReTu yaw: "));
    //Serial.println(yaw_D_remote);
  }
}
...
void printLoopRate() {
    ...
    Serial.print(F("  Kp = "));
    Serial.print(Kp_roll_rate[5]);
    Serial.print(F("  Ki = "));
    Serial.println(Ki_roll_rate[5]);
  }
}
